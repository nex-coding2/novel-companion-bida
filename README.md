...
2. 下载最新版本的 `novel-companion-0.2.0.vsix` 文件
...
- **导出成品**：`笔搭: 导出全书` 可导出 Markdown / TXT / EPUB / DOCX
- **MCP Server**：`笔搭: 安装 MCP Server` 把项目数据以只读 MCP 工具暴露给 AI 客户端
- **本地加密保险箱**：`笔搭: 加密入箱` 用 AES-256-GCM 加密敏感草稿
- **AI 直连（可选）**：配置 OpenAI 兼容端点后可一键校对章节，结果仍走审阅面板由作者确认
